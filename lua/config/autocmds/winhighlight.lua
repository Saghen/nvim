...
vim.api.nvim_create_autocmd({ 'BufEnter', 'WinEnter' }, {
  callback = function()
    if not vim.tbl_contains(shaded_filetypes, vim.bo.filetype) then return end
    vim.cmd(
    ...
    vim.schedule(function()
      ...
      if vim.bo.filetype == 'AvanteInput' then vim.schedule(function() vim.cmd('startinsert') end) end
    end)
  end,
})
